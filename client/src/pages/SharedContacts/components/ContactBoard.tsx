    ...
    <div className='w-full h-[800px] border rounded-md shadow-md flex flex-col'>
      <div className='sticky top-0 bg-white z-10 p-4 flex justify-between items-center border-b'>
        <div className='flex flex-row w-full gap-2'>
          <Input type='search' onChange={(e) => handleChange(e.target.value)} placeholder='Search contacts...' className='w-1/3' />
        </div>
        {props.children}
      </div>
        ...
        <div className='space-y-6'>
          {contacts.length === 0 ? (
            <div className='flex justify-center items-center'>
              <p className='text-lg font-semibold text-gray-500'>No contacts</p>
            </div>
          ) : (
            alphabet.map(letter => {
                      ...
                      (contact: TNewContact, index: number) => (
                        <Card
                          ...
                          <Avatar className='w-12 h-12 rounded-full'>
                            <AvatarImage src={contact.contact_profile_photo} />
                            <AvatarFallback>
                          ...
                          <CardContent className='p-0'>
                            ...
                            <p className='text-xs text-gray-700 font-bold italic'>
                              Contact is {contact.is_private ? 'Private' : 'Shared'} 
                            </p>
                          </CardContent>
                          <div className='w-full h-full flex justify-end'>
                            <div onClick={() => handleEdit(contact)}>
                              <Tooltip>
                                <TooltipTrigger><Eye /></TooltipTrigger>
                                <TooltipContent>
                                  <p>View</p>
                                </TooltipContent>
                              </Tooltip>
